...
const objLat = (obj) => {
    let capitalizeFirstName = obj.firstName.charAt(0).toUpperCase() + obj.firstName.slice(1);
    let capitalizeLastName = obj.lastName.charAt(0).toUpperCase() + obj.lastName.slice(1);
    return `my name is ${capitalizeFirstName} ${capitalizeLastName} I am ${obj.age} YO, and I love ${obj.hobby}.`;
...
const cvFormatter = (arr) => {
     const filtered = arr.filter((app) => app.yearsOfExperience > 1)
    .map((app) => {
      return {
        fullName: (app.firstName + " " + (app.lastName || "")).trimEnd(),
        tech: app.tech,
      };
    });
  return filtered;
